...
## Key User Tasks
1. Rename both project and proxy adapter from `AvgFreqCalcAdapter` to name more relevant to algorithm intent. For example, if calculation algorithm intent is to calculate power, e.g., MW, MVA, and MVAR, rename project file, adapter file and class name (in code) to `PowerCalcAdapter`.
2. Add custom properties specific to the Python calculation algorithm configuration to proxy adapter. These configuration values will be serialized and sent to Python adapter when algorithm is started.
